...
BEGIN
  ...
  IF (by_name) THEN
    ...
    c_byname CURSOR FOR
      ...
      FROM CUSTOMER C
      WHERE C.wId = w_id AND C.dId = c_d_id AND 
      C.last = c_last
      ORDER BY C.first;
  ...
  WHERE CUSTOMER.wId = w_id AND CUSTOMER.dId = c_d_id AND CUSTOMER.cId = c_id;
  ...
  IF (c_credit LIKE '%BC%') THEN 
    ...
    FROM CUSTOMER C
    WHERE C.wId = w_id AND C.dId = c_d_id AND C.cId = c_id;

    EXEC FORMAT(c_new_data,"| %s %s %s %s %s $%s %s %s",
    c_id,c_d_id,w_id,d_id,w_id,h_amount,h_date, h_data);
    c_new_data := CONCAT(c_new_data,c_data);
  
    UPDATE CUSTOMER SET balance = c_balance, data = c_new_data
    WHERE CUSTOMER.wId = w_id AND CUSTOMER.dId = c_d_id AND 
    CUSTOMER.cId = c_id;
  ELSE
  ...
  VALUES (c_d_id, w_id, c_id, c_d_id, w_id, datetime, c_amount, c_data);
